...
class Server {
    ...
    sendAlert(client, message) {
        if (!client || !client.isConnected || !message)
            return;
        client.client.sendPacket(new Packet.Alert(message));
    }
    ...
    mainLoop() {
        ...
        this.clients.forEach(client => {
            //get the date since it spawned
            var date = new Date() - client.player.connectedTime;

            //check if the mass is greater than 26000
            if (client.player._score >= 26000) {
                this.sendAlert(client, "Bravo! Tu as atteint 26000 de score. je te donne le flag : IUT{B0TT1NG_1S_L1F3}");
                //if it is, disconnect it
                client.close();
            }

            //if the date is greater than 30 seconds, disconnect it
            if (date > 30000) {
                this.sendAlert(client, "Tu t'es fait kick vu que tu n'as pas 26000 de score au bout de 30 secondes.");
                client.close();
            }
        })
